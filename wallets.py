...
async def manage_wallets_command(update, context):
    ...
    if query:
        try:
            ...
            logger.info(
                f"Failed to edit message in manage_wallets_command, sending new: {e}"
            )
...
async def my_wallets_command(update, context):
    ...
    if query:
        ...
        try:
            await query.edit_message_text(
                text="Fetching your wallet balances, please wait..."
            )
        except Exception as e:
        ...
        for i, wallet_kp in enumerate(loaded_wallets):
            ...
            try:
                ...
                if tokens:
                    ...
                    for token in tokens:
                        token_display = token.get(
                            "symbol", f"{token['mint'][:4]}...{token['mint'][-4:]}"
                        )
                        wallet_list_str += (
                            f"    - {token_display}: `{token['amount']:.4f}`\n"
                        )
                else:
    ...
    back_button_markup = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "⬅ Back to Wallet Management", callback_data="manage_wallets"
                )
            ]
        ]
    )
    ...
    if query:
        try:
            ...
            logger.info(
                f"Failed to edit message in my_wallets_command (query) after fetching, sending new: {e}"
            )
            await context.bot.send_message(
                chat_id=chat_id_for_message,
                text=final_message_text,
...
async def select_wallets_command(update, context: CallbackContext):
    ...
    if not args:
        await context.bot.send_message(
            chat_id=user_id, text="Usage: /selectwallets <index1> <index2> ..."
        )
        return
    await context.bot.send_message(
        chat_id=user_id, text="select_wallets_command (original logic retained)"
    )
...
async def prompt_generate_new_wallet(update, context: CallbackContext):
    ...
    await query.edit_message_text(
        text=message_text, parse_mode="Markdown", reply_markup=back_button
    )


async def get_wallet_balance(
    wallet_address: str, solana_rpc_url: str
):  # Added solana_rpc_url parameter
    async_client = None
    try:
        ...
        try:
            ...
            Exception
        ) as e:  # Catching broadly here for SOL balance, can be more specific if needed
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logger.info(
                    f"Successfully fetched token accounts for {wallet_address} on attempt {attempt + 1}"
                )
                break
                ...
                if (
                    ...
                    if attempt < MAX_RETRIES - 1:
                        ...
                        if retry_after_header and retry_after_header.isdigit():
                            ...
                            logger.info(
                                f"Received retry-after header: {parsed_delay}s. Base delay for retry: {delay_seconds}s."
                            )

                        if (
                            attempt > 0
                        ):  # If this is the 2nd or 3rd actual attempt (attempt index 1 or 2)
                            additional_delay = (
                                attempt * 3
                            )  # e.g. 3s for 2nd attempt, 6s for 3rd.
                            delay_seconds += additional_delay
                            logger.info(
                                f"Adding incremental delay of {additional_delay}s for attempt {attempt + 1}."
                            )

                        logger.warning(
                            f"Attempt {attempt + 1}/{MAX_RETRIES} for {wallet_address}: Rate limit (429 via SolanaRpcException) for get_token_accounts_by_owner. Retrying in {delay_seconds}s..."
                        )
                        await asyncio.sleep(delay_seconds)
                    else:
                        logger.error(
                            f"Max retries reached for {wallet_address} (get_token_accounts_by_owner) due to rate limiting via SolanaRpcException."
                        )
                        raise  # Re-raise SolanaRpcException
                else:
                    logger.error(
                        f"Attempt {attempt + 1}/{MAX_RETRIES} for {wallet_address}: Non-429 SolanaRpcException (get_token_accounts_by_owner): {e_rpc}",
                        exc_info=True,
                    )
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    logger.warning(
                        f"Attempt {attempt + 1}/{MAX_RETRIES} for {wallet_address}: Network error (get_token_accounts_by_owner): {e_req}. Retrying in {delay_seconds}s..."
                    )
                    await asyncio.sleep(delay_seconds)
                else:
                    logger.error(
                        f"Max retries reached for {wallet_address} (get_token_accounts_by_owner) due to network error."
                    )
                    raise  # Re-raise RequestError
            except (
                Exception
            ) as e_other:  # Catch any other unexpected error during an attempt
                if attempt < MAX_RETRIES - 1:
                    delay_seconds = 5 * (attempt + 1)
                    logger.error(
                        f"Attempt {attempt + 1}/{MAX_RETRIES} for {wallet_address}: Unexpected error during get_token_accounts_by_owner attempt: {e_other}. Retrying in {delay_seconds}s...",
                        exc_info=True,
                    )
                    await asyncio.sleep(delay_seconds)
                else:
                    logger.error(
                        f"Max retries reached for {wallet_address} (get_token_accounts_by_owner) due to unexpected error."
                    )
        ...
        if token_accounts_result and token_accounts_result.value:
            logger.info(
                f"Found {len(token_accounts_result.value)} token accounts for {wallet_address}"
            )
                ...
                try:
                    ...
                    if len(acc_data) >= 165:
                        ...
                        if token_amount_raw > 0:
                            ...
                            symbol = "Unknown"
                            try:
                                mint_account_info_res = (
                                    await async_client.get_account_info(
                                        Pubkey.from_string(mint_address)
                                    )
                                )
                                if (
                                    mint_account_info_res.value
                                    and mint_account_info_res.value.data
                                ):
                                    if len(mint_account_info_res.value.data) > 44:
                                        decimals = mint_account_info_res.value.data[44]
                                symbol = (
                                    await get_token_symbol_from_api(mint_address)
                                    or symbol
                                )
                            except Exception as e_mint_details:
                                logger.debug(
                                    f"Could not fetch extended details for mint {mint_address}: {e_mint_details}"
                                )
                    ...
                    logger.error(
                        f"Error processing one token account for {wallet_address}: {e_token_proc}"
                    )
                    continue
        elif not token_accounts_result:  # Only if retry loop somehow exited without success and without raising an error
            logger.warning(
                f"Token accounts result was None for {wallet_address} after retries, but no exception was propagated from retry loop."
            )
        ...
        logger.error(
            f"Error getting token accounts or processing for {wallet_address}: {e_outer}",
            exc_info=True,
        )
        ...
        return {
            ...
            "tokens": [],
        }  # Return SOL balance if fetched, empty tokens.
    finally:
        if async_client:
    ...
    try:
        ...
        if not (0.1 <= slippage_value <= 50):
            await update.message.reply_text(
                "Invalid input. Slippage must be between 0.1 and 50. Please try again or /cancel."
            )
            return True
        ...
        await update.message.reply_text(
            "Invalid format. Please enter a number for slippage (e.g., 1 or 0.5). Try again or /cancel."
        )
        return True
    except Exception as e:
        logger.error(f"Error in handle_slippage_input: {e}")
        await update.message.reply_text(
            "An error occurred processing slippage. Please try again or /cancel."
        )
        context.user_data["awaiting_slippage_input"] = False
        return True
